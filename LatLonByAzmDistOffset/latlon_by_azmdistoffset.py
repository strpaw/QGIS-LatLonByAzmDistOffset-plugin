...
from latlon_by_azmdistoffset_dialog import LatLonByAzmDistOffsetDialog
import os.path
...
def checkAzmDistOffset(azm, dist, dist_UOM, offset, offset_UOM, offset_side):
    valid_UOM = ['m', 'KM', 'NM', 'FEET', 'SM']
    isValid = True
    errMsg = ''
    ...
    if dist_UOM not in valid_UOM:
        isValid = False
        errMsg += '*Distance UOM error*'
    if checkDistance(offset) == NOT_VALID:
        isValid = False
        errMsg += '*Offset value error*'
    if offset_UOM not in valid_UOM:
        isValid = False
        errMsg += '*Offset UOM error*'
    if offset_side not in ['LEFT', 'RIGHT']:
        isValid = False
        errMsg += '*Offset side error*'
...
class LatLonByAzmDistOffset:
    ...
    def __init__(self, iface):
        ...
        self.distanceUnit = UOM_M
        self.offset = None
        self.offset_m = None
        self.offsetUnit = UOM_M
        self.offsetSide = ''
        self.outLyrSpName = ''
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'LatLonByAzmDistOffset_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&LatLonByAzmDistOffset')
        # TODO: We are going to let the user set this up in a future iteration
        self.toolbar = self.iface.addToolBar(u'LatLonByAzmDistOffset')
        self.toolbar.setObjectName(u'LatLonByAzmDistOffset')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('LatLonByAzmDistOffset', message)
    ...
    def add_action(
        ...
        self.dlg = LatLonByAzmDistOffsetDialog()
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/LatLonByAzmDistOffset/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'LatLonByAzmDistOffset'),
            callback=self.run,
            parent=self.iface.mainWindow())

        self.dlg.pbAddPoint.clicked.connect(self.addSpPoint)
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&LatLonByAzmDistOffset'),
                action)
    ...
    def getDistanceUnit(self):
        """ Gets distance unit """
        if self.dlg.cbSpDistanceUnit.currentText() == 'm':
            unit = UOM_M
        elif self.dlg.cbSpDistanceUnit.currentText() == 'KM':
            unit = UOM_KM
        elif self.dlg.cbSpDistanceUnit.currentText() == 'NM':
            unit = UOM_NM
        elif self.dlg.cbSpDistanceUnit.currentText() == 'feet':
            unit = UOM_FEET
        elif self.dlg.cbSpDistanceUnit.currentText() == 'SM':
            unit = UOM_SM
        return unit
        
    def getOffsetUnit(self):
        """ Gets offset unit """
        if self.dlg.cbSpOffsetUnit.currentText() == 'm':
            unit = UOM_M
        elif self.dlg.cbSpOffsetUnit.currentText() == 'KM':
            unit = UOM_KM
        elif self.dlg.cbSpOffsetUnit.currentText() == 'NM':
            unit = UOM_NM
        elif self.dlg.cbSpOffsetUnit.currentText() == 'feet':
            unit = UOM_FEET
        elif self.dlg.cbSpOffsetUnit.currentText() == 'SM':
            unit = UOM_SM
        return unit
        
    def getOffsetSide(self):
        """ Gets offset side """
        if self.dlg.cbSpOffsetSide.currentText() == 'left':
            side = 'LEFT'
        elif self.dlg.cbSpOffsetSide.currentText() == 'right':
            side = 'RIGHT'
        return side
        
    def checkAzmDistOffsetInput(self):
        checkResult = True
        errMsg = ''
        ...
        self.offset = self.dlg.leEpOffset.text()
        ...
        if self.bearing == '':
            errMsg += 'Enter azimuth/bearing!\n'
            checkResult = False
        else:
            if checkAzimuth(self.bearing) == NOT_VALID:
                errMsg += 'Azimuth wrong format!\n'
                checkResult = False
            
        if self.distance == '':
            errMsg += 'Enter distance!\n'
            checkResult = False
        else:
            if checkDistance(self.distance) == NOT_VALID:
                errMsg += 'Distance wrong format!\n'
                checkResult = False
        ...
        if self.offset == '':
            ...
            if checkDistance(self.offset) == NOT_VALID:
                errMsg += 'Offset wrong format!\n'
                checkResult = False
        ...
        if checkResult == True:
            ...
            self.offsetUnit = self.getOffsetUnit()
            self.offset_m = toMeters(float(self.offset), self.offsetUnit)
            self.offsetSide = self.getOffsetSide()
        else:
            QMessageBox.critical(w, "Message", errMsg)
        return checkResult

    def addSpPoint(self):
        if self.checkAzmDistOffsetInput() == True:
            epLatDD, epLonDD = dist_azm_orth_offset2latlon(self.rpLatDD, self.rpLonDD, self.azimuth, self.distance_m,
    ...
    def checkAzmDistOffsetCSVInput(self):
        ...
        hToCheck = 'P_NAME;AZM_BRNG;DIST;DIST_UOM;OFFSET;OFFSET_UOM;OFFSET_SIDE'
        ...
        if rpLatDMS != '':
            self.rpLatDD = parseDMS2DD(rpLatDMS, V_LAT)
            if self.rpLatDD == NOT_VALID:  
                errMsg += 'Reference point latitude wrong format!\n'
                checkResult = False
        ...
        if rpLonDMS != '':
            self.rpLonDD = parseDMS2DD(rpLonDMS, V_LON)
            if self.rpLonDD == NOT_VALID:  
                errMsg += 'Reference point longitude wrong format!\n'
                checkResult = False
    ...
    def calcCSV(self):
        if self.checkAzmDistOffsetCSVInput():
            errNotes = ''
                ...
                with open(self.outputFile, 'w') as outCSV:
                    ...
                    for row in reader:
                        try: # Try to read line according to field names
                            validInputLine, errNotes = checkAzmDistOffset(row['AZM_BRNG'], row['DIST'], row['DIST_UOM'], 
                                                                          row['OFFSET'], row['OFFSET_UOM'], row['OFFSET_SIDE'])
                            if validInputLine: # azimuth or brng and distance are valid
                                ...
                                dist_m = toMeters(float(row['DIST']), row['DIST_UOM'])
                                offs_m = toMeters(float(row['OFFSET']), row['OFFSET_UOM'])
                                epLatDD, epLonDD = dist_azm_orth_offset2latlon(self.rpLatDD, self.rpLonDD, azm, dist_m,
                                                                               offs_m, row['OFFSET_SIDE'])
